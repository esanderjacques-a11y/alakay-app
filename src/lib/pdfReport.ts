...
export async function exportAnalysisPdf(options: {
  ...
  function drawFooter() {
    ...
    pdf.text(
      pdfSafe(`${pdfBrandName(t.appName)} · ${t.reportSubtitle}`),
      margin,
      pageHeight - 7
    );
  ...
  function drawCover() {
    ...
    if (meta.length === 0 && reportMeta?.details?.length) {
      for (const detail of reportMeta.details) {
        const sep = detail.indexOf(":");
        if (sep > 0) {
          meta.push({
            label: detail.slice(0, sep).trim(),
            value: detail.slice(sep + 1).trim(),
          });
        } else if (detail.trim()) {
          meta.push({ label: "", value: detail.trim() });
        }
      }
    }
    ...
    y = drawPdfReportHeader({
      ...
      includeLogo: exportSections.includeLogo,
      logoData,
      startY: 14,
    });
  }
